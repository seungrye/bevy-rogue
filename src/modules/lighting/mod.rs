...
/// 기억 타일의 **망각 감쇠 계수** — 지수형(인지심리 Ebbinghaus 곡선 형태).
/// 사용자 디자인: "500 턴이면 거의 배경과 구분이 안 될 정도". 시상수 τ=100 으로
/// Δ=500 에서 exp(-5) ≈ 0.0067 → 1% 미만 → 시각적으로 배경.
///
/// 곡선: `exp(-Δturn / 100)`.
///   - Δturn = 0   → 1.0    (방금 본 — 감쇠 없음)
///   - Δturn = 50  ≈ 0.607  (살짝 어둑)
///   - Δturn = 100 ≈ 0.368  (절반점)
///   - Δturn = 200 ≈ 0.135
///   - Δturn = 300 ≈ 0.050
///   - Δturn = 500 ≈ 0.0067 (배경과 사실상 구분 불가)
///   - Δturn → ∞   → 0.0
pub fn memory_fade_factor(turns_since_seen: u32) -> f32 {
    (-(turns_since_seen as f32) / 100.0).exp()
}
