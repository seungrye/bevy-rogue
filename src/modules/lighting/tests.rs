...
fn dimming_app_with_light(
    ...
    // 타일 엔티티 — brightness 는 per-entity 컴포넌트로 부착.
    app.world.spawn((
        Text::from_section("x", TextStyle { color: Color::rgb(0.123, 0.123, 0.123), ..default() }),
        vis,
        TileEntity { x: 0, y: 0 },
        crate::modules::map::TileBrightness(brightness),
        crate::modules::map::TileLastSeen::default(),
    ));
...
fn 디밍시스템은_이미_같은_색이면_재대입하지_않는다() {
    ...
    app.world.spawn((
        ...
        Visibility::Visible,
        TileEntity { x: 0, y: 0 },
        crate::modules::map::TileBrightness(1.0),
        crate::modules::map::TileLastSeen::default(),
    ));
    app.add_systems(Update, apply_light_dimming);
    app.update();
...
fn 디밍시스템은_플레이어_거리에_따라_시야_안_타일을_더_어둡게_칠한다() {
    ...
    // 멀리 있는 타일(2,0) 만 엔티티로 검사 — brightness 0.5 (d=2 falloff 시뮬).
    app.world.spawn((
        Text::from_section("x", TextStyle { color: Color::rgb(0.0, 0.0, 0.0), ..default() }),
        Visibility::Visible,
        TileEntity { x: 2, y: 0 },
        crate::modules::map::TileBrightness(0.5),
        crate::modules::map::TileLastSeen::default(),
    ));
    app.add_systems(Update, apply_light_dimming);
    app.update();
...
#[test]
fn 기억감퇴_델타_100턴이면_factor는_약_0_3679이다() {
    // exp(-1) ≈ 0.36788 — 시상수 100. 100 턴이면 절반쯤(약 37%) 까지 감쇠.
    let f = memory_fade_factor(100);
    assert!((f - (-1.0f32).exp()).abs() < 1e-6);
    assert!((f - 0.36788).abs() < 1e-3, "100턴은 약 0.37");
}

#[test]
fn 기억감퇴_델타_200턴이면_factor는_약_0_1353이다() {
    // exp(-2) ≈ 0.13534.
    let f = memory_fade_factor(200);
    assert!((f - 0.13534).abs() < 1e-3, "200턴은 약 0.135");
}

#[test]
fn 기억감퇴_델타_500턴이면_거의_배경이다() {
    // 사용자 디자인: '500 턴이면 거의 배경과 구분이 안될 정도'.
    // exp(-5) ≈ 0.0067 → 1% 미만.
    let f = memory_fade_factor(500);
    assert!(f < 0.01, "500턴은 1% 미만 — 사실상 배경");
    assert!(f > 0.0, "지수형이라 정확한 0 은 아님(점근)");
}

#[test]
fn 기억감퇴는_매우_큰_델타에서_0에_수렴한다() {
    // u32 범위 안에서 큰 값들도 0 에 매끄럽게 수렴해야 한다.
    let a = memory_fade_factor(1_000);
    let b = memory_fade_factor(5_000);
    let c = memory_fade_factor(50_000);
    assert!(a < 1e-3, "Δ=1000 이면 거의 0");
...
/// fov 테스트 헬퍼 — map 전체에 대해 TileEntity 와 grid 를 세팅한다.
/// update_fov 가 TileEntityGrid 로 lookup 해 brightness/last_seen 컴포넌트를 갱신하므로
/// 모든 타일 entity 가 필요.
fn spawn_full_tile_grid(app: &mut App, width: usize, height: usize) {
    use crate::modules::map::{TileBrightness, TileLastSeen, TileEntityGrid};
    let mut grid = TileEntityGrid::default();
...
fn last_seen_at(app: &mut App, x: usize, y: usize) -> Option<u32> {
    use crate::modules::map::{TileLastSeen, TileEntityGrid};
    let entity = app.world.resource::<TileEntityGrid>().at(x, y).unwrap();
    app.world.get::<TileLastSeen>(entity).unwrap().0
}
...
fn fov는_보인_타일의_last_seen_turn을_현재_글로벌턴으로_갱신한다() {
    ...
    spawn_full_tile_grid(&mut app, 30, 30);
    ...
    let _ = map_idx;
    assert_eq!(last_seen_at(&mut app, 10, 10), Some(42), "보인 타일은 GlobalTurn 으로 갱신");
}

#[test]
fn fov는_보이지_않는_타일의_last_seen_turn을_유지한다() {
    use crate::modules::map::{Map, MapResource, GlobalTurn, TileLastSeen, TileEntityGrid};
    use crate::modules::player::{Facing, Player};
    let mut map = Map::new(30, 30);
    for y in 0..30 { for x in 0..30 { map.set_tile(x, y, TileKind::Floor); } }
    // 멀리 떨어진 타일에 이전에 본 기록을 심어 둔다 (map 의 revealed 만 set, 컴포넌트는 spawn 후).
    ...
    spawn_full_tile_grid(&mut app, 30, 30);
    // far 타일의 TileLastSeen 을 미리 세팅 (이전에 본 기록).
    {
        let grid = app.world.resource::<TileEntityGrid>();
        let entity = grid.at(29, 29).unwrap();
        app.world.get_mut::<TileLastSeen>(entity).unwrap().0 = Some(10);
    }
    ...
    assert_eq!(last_seen_at(&mut app, 29, 29), Some(10),
...
fn 디밍시스템은_기억타일의_경과_턴에_따라_더_어둡게_칠한다() {
    use crate::modules::map::{GlobalTurn, TileBrightness, TileLastSeen};
...
fn 디밍시스템은_GlobalTurn_리소스가_없어도_기존_0_3_factor를_유지한다() {
    use crate::modules::map::{TileBrightness, TileLastSeen};
    let mut app = App::new();
    let mut map = Map::new(1, 1);
    map.set_tile(0, 0, TileKind::Floor);
    map.tiles[0].visible = false;
...
// ── 세이브 호환 (MapTile 직렬화 — brightness/last_seen 은 per-entity 컴포넌트로 분리됨) ──

#[test]
fn 기존_세이브의_MapTile_직렬화는_kind_revealed_visible만으로_복원된다() {
...
#[test]
fn MapTile은_세_필드_round_trip한다() {
    use crate::modules::map::{MapTile, TileKind};
    let t = MapTile { kind: TileKind::Floor, revealed: true, visible: true };
    let s = serde_json::to_string(&t).unwrap();
    let back: MapTile = serde_json::from_str(&s).unwrap();
    assert_eq!(back, t, "round-trip 보존");
    assert!(s.contains("kind") && s.contains("revealed") && s.contains("visible"));
}
