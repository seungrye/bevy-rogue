...
fn split_rect(rect: Rect, rooms: &mut Vec<Rect>, depth: usize) {
    ...
    if split_h {
        if rect.y2.saturating_sub(rect.y1) < 5 { rooms.push(rect); return; }
        let sy = rng.gen_range(rect.y1 + 2..rect.y2 - 2);
        ...
        if rect.x2.saturating_sub(rect.x1) < 5 { rooms.push(rect); return; }
        let sx = rng.gen_range(rect.x1 + 2..rect.x2 - 2);
