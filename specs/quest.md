...
- **빈 `dialog` 규칙**: 코드의 마지막 대화 분기
  ...
  state 등록·on_interact 실행으로 가지 않는다. 따라서:
  - `initial_phase` — **dialog 최소 1 줄 필수**. 빈 dialog 면 NPC 부딪힘
    이 영원히 등록되지 않는다.
  - **NPC 부딪힘으로 진행되는 phase** (`on_interact` 비어있지 않음) —
    dialog 최소 1 줄 필수. 빈 dialog 면 진행 액션이 트리거되지 않는다.
  - **terminal phase** (`on_interact` 와 `auto_advance` 모두 비어있음) —
    빈 dialog 안전. 그 phase 는 어차피 더 이상 진행 안 됨. 현재
  ...
  - **`auto_advance` 만으로 진행되는 phase** — NPC 대화 없이 자동 전진
