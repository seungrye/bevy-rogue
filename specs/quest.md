...
- **퀘스트 패널 등록 시점**: NPC 와 첫 부딪힘 만으로는 `QuestState.phases`
  ...
  시점에 등록 + `on_interact` 액션 실행. 즉 인사말만 듣고 퀘스트 패널
  (Q 키) 에 뜨는 일은 없다 — 핵심 의뢰 설명까지 다 들어야 등록된다.
  대화 진행은 `Villager.quest_dialogue_idx` 가 추적하므로 state 등록과
  무관하게 dialog 순회가 가능하다.
- **`initial_phase` 의 `dialog` 는 최소 1 줄 필수**: 코드의 마지막 대화
  분기 (`!dialog.is_empty() && idx + 1 >= dialog.len()`) 가 빈 dialog 시
  state 등록·on_interact 실행으로 가지 못한다. `initial_phase` 가 빈
  dialog 면 NPC 와 부딪혀도 영원히 등록되지 않는 상태가 된다. 즉시
  등록·자동 액션이 의도라도 dialog 에 한 줄 (예: 단순 인사) 은 둬야
  한다. 후속 phase 의 빈 dialog 는 무방 — `auto_advance` 로 진입한 phase
  는 NPC 대화 없이도 처리된다.
