...
| `SpawnGuards { count }` | 현재 맵에 잠입 가드 `count` 마리 스폰 (`SpawnGuardEvent` → `handle_spawn_guards`). 가드 스탯은 플레이어 현재 ATK/DEF/HP × `GUARD_POWER_MULT`(1.2). 잠입 퀘스트용 |
...
| `infiltration_quest` | 0.7 | 잠입 (그림자 속의 회수) |
| `vault_heist_quest` | 0.65 | 잠입 체이닝 (얼어붙은 금고) |
...
### 잠입 퀘스트 (`infiltration_quest`, `vault_heist_quest`)

방향 시야 위에 올린 잠입 콘텐츠. 가드에게 탐지되지 않고 목표 구역을 통과한다.
방향 FOV·가드 모델 설계는 [`stealth-and-directional-fov.md`](stealth-and-directional-fov.md) 참고.

- `그림자 속의 회수` (`infiltration_quest`) — giver: 촌장. 단일 잠입 구역.
- `얼어붙은 금고` (`vault_heist_quest`) — giver: 상인. infiltration 완료 후 이어지는 체이닝.
- 가드는 `QuestAction::SpawnGuards { count }` 로 배치, 스탯은 플레이어 현재
  effective ATK/DEF/HP × `GUARD_POWER_MULT`(1.2) — 늘 "조금 더 셈".
- 가드(또는 일반 몬스터)가 방향 시야로 플레이어를 탐지하면 활성 잠입 퀘스트의
  `stealth_blown` 플래그가 set. 목표 도달로 클리어는 가능하되 `stealth_blown`
  이면 완료 시 **무탐지 보너스 미지급** (보상 분기는 `FlagIs`/`HasFlag` 로 표현).
