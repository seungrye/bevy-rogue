...
미니맵에 퀘스트 제공자, 퀘스트 목표, 존 포털(맵 경계), 계단(내려가기/올라가기) 위치를
발견 후 지속 표시한다.
...
| `QuestTarget` | 자홍색 (1.0, 0.0, 1.0) | 퀘스트 목표가 배정될 때 |
...
- `QuestTarget`: 퀘스트 목표가 선택되어 `QuestState`에 저장될 때
- `Portal` / `StairDown` / `StairUp`: `map.visible_tiles[idx]` 가 true 인 포털 엔티티 타일

## 미니맵 렌더링

- 기존 타일 렌더링 패스 이후 마커 오버레이 패스를 추가한다
- 마커 픽셀 좌표 계산:
  ```
...
- 마커는 중심 픽셀과 상하좌우 픽셀을 포함하는 작은 십자 스탬프로 표시한다
...
- [x] 퀘스트 목표 위치가 `QuestTarget` 마커로 추가된다 (중복 추가 안 함)
- [x] 미니맵 렌더링 시 현재 존의 마커만 픽셀 스탬프로 표시된다
