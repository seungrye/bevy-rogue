...
- [ ] CLI `--glyph-style <ascii|unicode|icon>` 으로 초기 스타일 지정 (기본값: `ascii`)
- [ ] `G` 키로 ascii → unicode → icon → ascii 순서로 순환
- [ ] 스타일 전환 시 월드에 존재하는 모든 아이템 엔티티의 글리프·폰트가 즉시 갱신된다
- [ ] 스타일 전환 시 로그에 현재 스타일 이름이 출력된다
- [ ] 신규 드롭 아이템은 현재 스타일로 스폰된다
