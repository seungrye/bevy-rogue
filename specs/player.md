...
INVENTORY 섹션은 번호+아이콘+이름+수치. 무기/방어구는 **[등급] 이름 (롤된 ATK/DEF)**
형태로 레어도 색을 적용하고, 장착 항목엔 `[장착]` 태그 (밝은 녹색).
아이콘(rpg-awesome) 과 한글 (NanumSquareNeo) 은 별도 TextSection 으로 혼합 렌더링.
(레어도·롤된 스탯 모델은 [`specs/item-random-stats.md`](item-random-stats.md).)

### 전투 스탯
- 무기 장착 시 공격력 = 무기의 롤된 값(`effective_attack`), 없으면 범위 중앙값 (기본 ATK 대체).
- 방어구 장착 시 기본 방어력 + 롤된 보너스.
- 해제 시 기본 스탯 복원.
